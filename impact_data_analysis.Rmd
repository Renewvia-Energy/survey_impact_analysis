...
kenya_paired_enc <- paired_enc %>% filter(country_post == 2)
...
plot_reg <- function(data, feature, dependent, 
  ...
  plot <- plot + 
          labs(subtitle = paste("Estimate = ",est, " , ", 
                                    'p-value = ', p_val), 
                      y=y_axis, x=x_axis) + theme_classic()
  return(plot)
}
```
...
survey_names <- c(
                    `hs_pre` = "Household Pre-Survey",
                    `hs_post` = "Household Post-Survey",
                    `ci_post` = "Commercial & Institution Post-Survey"
                    )
q_plot <- q_dist %>% ggplot(aes(y=per, x=Axis, fill=Axis)) +
                              ...
                              geom_label_repel(
                                aes(label = glue("({per}%)"),
                                     fill = Axis),
                                    show.legend = FALSE) +
                              labs(fill = "Survey", y="Percent of Respondents (%)"
...
g_plot <- g_dist %>% ggplot(aes(y=per, x=Gender, fill=Gender)) +
                              ...
                              geom_label_repel(
                                aes(label = glue("({per}%)"),
                                     fill = Gender),
                                    show.legend = FALSE) +
                              labs(y="Percenta of Respondents (%)"
                                   #title="Distribution of Survey Questions"
                                   )+ 
                              #coord_flip() +
                              facet_grid(
...
compare_pre_post(df_pre=initial_pre, df_post=post_clean,
                 ...
                 var_type="non_cont", subplot_type="bar_simple",
...
community_reg(df=kenya_paired_enc, 
              ...
              diff_type="direction", customer_type="hs", 
              y_axis= "Households",
              title_sig="Increase of Avg. Household Income in Communities") 
```
...
consumption_reg(df=kenya_temp,  reg_type ="linear",
                dep="avg_household_income_post", 
                ind="avg_mon_consumption_cum",
                exclude_outliers = TRUE, outliers = "dependent",
                #calc_diff=TRUE, diff_type = "magnitude",
                x_axis="Avg. Monthly Energy Consumption Cumulative (kWh)", 
...
```{r}
# income_rectified <- read.csv("kenya_incomes_rectified.csv")
# paired_enc_df <- merge(paired_enc, income_rectified,
#                           by.x = 'renewvia_id', 
#                           by.y = 'Customer.Account.Number')
# write.csv(paired_enc_df, file='paired_enc_rect.csv',
#           fileEncoding = "UTF-8")
```

```{r}
# write.csv(ci_enc, file='ci_enc_rect.csv',
#           fileEncoding = "UTF-8")
```
...
re_names <- c(
  `water_collection_time` = "Water Collection Time", 
  `cooking_fuel_collection_time` = "Cooking Fuel Collection Time"
                    )
...
box_plot <- box_data %>% ggplot(aes(x = Period, 
                        ...
                        #theme(legend.text=element_text(size=6))
box_plot + remove_x + theme(legend.position = 'bottom', 
                        legend.text=element_text(size=6)) +
  scale_fill_manual(values=c("#f79d27", "#ef641e")) + 
  theme_classic()
```
...
box_plot <- box_data %>% ggplot(aes(x = Period, 
                        ...
                        facet_grid(~factor(var,
                                   ...
                                   switch = "both") + 
                        theme_classic() +
                        scale_fill_manual(values=c("#ef641e", "#87CEEB")) +
                        theme(legend.position = 'top', 
                       legend.text=element_text(size=12),
                       strip.background = element_blank(),
                       strip.text = element_text(size = 12),
                       axis.text=element_text(size=12), #change font size of axis text
                       axis.title=element_text(size=12)) + remove_x
...
violins(data = paired_enc, 
        ...
        new_names = c("Water Collection Time", 
                      "Cooking Fuel Collection Time"),
        show_median = "point"
        #title="Gender Equality - Paired Testing Results"
        )
...
re_names <- c(
  ...
  `water_collection_travel_distance` = "Water Collection Distance"
                    )
box_data$Period <- factor(box_data$Period , levels=c("pre", "post"))
box_plot <- box_data %>% ggplot(aes(x = Period, 
                                    y = value, fill=Period)) + 
                       ...
                       axis.title=element_text(size=8)) + remove_x
...
violins(data = paired_enc, 
        ...
        new_names = c("Light Hours",
                      "No. of Appliances",
                      "Phone Charging Distance",
...
violins(data = paired_enc, 
        ...
        new_names = c("No. of Kerosene Lamps"),
        show_median = "point"
        #title="Health - Paired Testing Result(s)"
        )
```
...
re_names <- c(
  ...
  # `appliances_count` = "No. of Appliances", 
  # `phone_charge_travel_distance` = "Phone Charging Distance",
  `phone_charge_cost` = "Phone Charge Cost",
  `applicances_charging_cost` = "Appliance Charge Cost",
  `cooking_energy_cost` = "Cooking Energy Cost", 
  `kerosene_lamps_cost` = "Kerosene Lamps Cost",
  `water_cost` = "Water Cost",
  `avg_household_income_nigeria` = "Nigeria Incomes",
  `avg_household_income_kenya` = "Kenya Incomes"
                    )
box_data$Period <- factor(box_data$Period , levels=c("pre", "post"))
box_plot <- box_data %>% ggplot(aes(x = Period, 
                                    y = value, fill=Period)) + 
                       ...
                       axis.title=element_text(size=6)) + remove_x
...
violins(data = paired_merged, 
        ...
        new_names = c( 
          ...
          "Kenya Incomes"),
        show_median = "point",
        #title="Economic Activity - Paired Testing Result(s)"
        )
```
...
re_names <- c(
  ...
  # `phone_charge_travel_distance` = "Phone Charging Distance",
  `kerosene_lamps_count` = "No. of Kerosene Lamps",
  `phone_charge_cost` = "Phone Charging Cost"
                    )
box_data$Period <- factor(box_data$Period , levels=c("pre", "post"))
par(ps=12)
box_plot <- box_data %>% ggplot(aes(x = Period, 
