  ...
  if(var_type == "non_cont") {
      ...
      if(plot_type == "hist") {
        ...
        plot <- plot +
                ...
                theme_bw() +
                labs(title=plot_title, 
                     subtitle = paste("Mean = ", round(mu, 0), 
                                      "  Median = ", med), 
                     x=x_axis, y=y_axis) 
  }
  return(plot)
...
## D) Correlation - Dependent Variables

```{r}
corr_simple <- function(data,sig=0.1){
...
consumption_reg(df = paired_enc, reg_type ="linear",
                ...
                title="Avg. Monthly Energy Consumption vs. Difference in No. of Kerosene Lamps")
```
...
consumption_reg(df = sub, reg_type ="linear", ind="avg_mon_consumption_cum",
                dep= "health_offering_change_count", calc_diff=FALSE,
                exclude_outliers = TRUE, outliers = "feature", 
                title="Avg. Monthly Energy Consumption vs. New Health Services Offered")
```
...
```{r}
safe_1 <- paired_enc %>% filter(feel_safe_dark_pre == 1)
compare_paired(data=safe_1,
               var_name="feel_safe_dark",
               lgd_title="Score",
               plot_title="Paired: Feeling Safe When Dark")
```
