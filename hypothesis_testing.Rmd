...
```{r}
# Isolate the dependent variable
var <- hs_post_clean$kerosene_lamp_usage_change
...
dep_var_mat <- rownames_to_column(
  as.data.frame.matrix(prop.table(table(hs_post_clean$community, 
                                   hs_post_clean$kerosene_lamp_usage_change),
                                         margin = 1)
                                                  )
                                      , var = "community")
community_data <- merge(x=projects, 
...
```{r}
# Isolate the dependent variable
var <- hs_post_clean$household_income_change
...
dep_var_mat <- rownames_to_column(
  as.data.frame.matrix(prop.table(table(hs_post_clean$community, 
                                   hs_post_clean$household_income_change),
                                         margin = 1)
                                                  )
                                      , var = "community")
community_data <- merge(x=projects, y=dep_var_mat, by="community")
