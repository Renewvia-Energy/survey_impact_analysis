...
-   Visualizatin with graphs/charts
...
get_stats <- function(data_source, var_name,
  ...
  if (remove_outliers == TRUE) {
    outliers_removal(data=data_source, var=var_name)
  } 
...
plot_func <- function(df, var_name, var_type, 
  ...
  if(remove_outliers == TRUE) {
      outliers_removal(data=df, var=var_name)
  }
...
paired_testing <- function(data, var_name, test_type, 
  ...
  if(remove_outliers==TRUE) {
    data$temp <- abs(data[, post_name] - data[, pre_name])
    outliers_removal(data=data, var_name=temp)
  }
...
plot_reg <- function(data, feature, dependent, 
                     remove_outliers, which_outliers=NULL,
  ...
  if(remove_outliers == TRUE) {
      if(which_outliers=="dependent"){
        data <- outliers_removal(data=data, var_name=dependent)
        
      }else if(which_outliers=="feature"){
        data <- outliers_removal(data=data, var_name=feature)
        
      }else if(which_outliers=="all"){
        temp <- outliers_removal(data=data, var_name=feature)
        data <- outliers_removal(data=data, var_name=dependent)
      }
  }
...
```{r}
consumption_reg <- function(df, dep, ind="Energy.Consumption",title, reg_type, 
                            calc_diff=FALSE, 
                            exclude_outliers = FALSE,
                            outliers=NULL, 
                            to_remove=NULL) {
...
community_reg(df=paired_enc, 
              ...
              exclude_outliers = TRUE, outliers = "all",
...
paired_testing(paired_enc, var_name="cooking_fuel_collection_time", 
               test_type = "wilcoxon", plot_diff=TRUE, 
               plot_title="Difference Pre-Post of Cooking Fuel Collection Times")
```
...
community_reg(df=paired_enc, 
              ...
              var_type="ratio", calc_diff = TRUE,
              exclude_outliers = TRUE, outliers = "feature",
              title="Cumulative Differences in 
...
consumption_reg(df = df_reg, reg_type ="linear",
                ...
                exclude_outliers = TRUE, outliers = "feature",
                title="New Female Workers Change")
```
...
paired_testing(paired_enc, var_name = "light_hours_current", 
               plot_diff = TRUE, 
               plot_title = "Difference Pre-Post in Light Hours",
               test_type="ttest", remove_outliers=TRUE)
```
...
```{r}
# Multiply proportions by total numbers -> approximate the synthetic count by community
# Assumptions
```
...
compare_pre_post(df_pre=initial_pre, df_post=post_clean, 
                 ...
                 subplot_type="pie", 
                 lgd_title = "Does the clinic have electricity?",
                 title="Unpaired: Clinic Access To Electricity")

# Paired
compare_paired(data=paired_clean,
               var_name="clinic_electricity_access",
               lgd_title="Does the clinic have electricity?",
               plot_title="Paired: Clinic Access To Electricity")
```
...
compare_pre_post(df_pre=initial_pre, df_post=post_clean, 
                 ...
                 subplot_type="pie", 
                 lgd_title = "Does the Clinic have access to refrigeration?",
...
compare_paired(data=paired_clean,
               ...
               lgd_title="Does the Clinic have access to refrigeration?",
               plot_title="Paired: Clinic Access to Refrigeration")
```
...
plot_func(df=ci_clean, var_name="health_offering_change_count", 
          ...
          plot_type="bar_discrete", char_remove = 0)
```
...
#                 dep= "clean_drinking_water",
#                 calc_diff = TRUE,
#                 exclude_outliers = TRUE,
#                 outliers = "feature",
#                 title="Energy Consumption vs. Increase in
#                 Access to clean drinking water")
```
...
community_reg(df=paired_enc, calc_diff = TRUE,
              ...
              var_type="ratio", outliers = "feature", exclude_outliers = TRUE)
```
...
compare_pre_post(df_pre=initial_pre, df_post=post_clean, 
                 ...
                 subplot_type="pie", 
                 lgd_title = "Does the community have lights?",
...
compare_pre_post(df_pre=initial_pre, df_post=post_clean, 
                 ...
                 title="Unpaired: Home Exterior Lights")

# Paired
compare_paired(data=paired_clean,
               var_name="home_exterior_lights",
               lgd_title="Does the home have exterior lights?",
               plot_title="Paired: Home Exterior Lights")
```
...
paired_testing(gender_paired, var_name="feel_safe_dark", 
               test_type = "mcnemar", plot_diff=TRUE)
```
...
```{r}
# paired_enc$safe_diff <- ifelse(paired_enc$feel_safe_dark_pre < 
#                                paired_enc$feel_safe_dark_post, 1, 
#                                ifelse((paired_enc$feel_safe_dark_pre >
#                                        paired_enc$feel_safe_dark_post), -1, 0))
```
...
compare_pre_post(df_pre=initial_pre, df_post=post_clean, 
                 ...
                 title="Unpaired: Average Household Income")
```
